...
Begin:"""
    ...
    def _generate_chart_data(self, jobs):
        """Generate data for pie chart visualization - sorted by count, group small categories"""
        ...
        # Group categories under 20% into "Other"
        total_jobs = len(jobs)
        threshold = total_jobs * 0.20  # 20% threshold
        ...
        for cat, count in sorted_categories:
            if count >= threshold:
                filtered_categories.append((cat, count))
            else:
                other_count += count
        
        # Add "Other" if there are small categories
        if other_count > 0:
            filtered_categories.append(("Other", other_count))
        
        # Group locations under 20% into "Other"
        filtered_locations = []
        ...
        for loc, count in sorted_locations:
            if count >= threshold:
                ...
                other_location_count += count
        
        # Add "Other" if there are small locations
        if other_location_count > 0:
            filtered_locations.append(("Other", other_location_count))
        
        # Prepare category pie chart data (sorted, with Other)
        category_pie_data = {
            "labels": [item[0] for item in filtered_categories],
            "values": [item[1] for item in filtered_categories],
            "colors": self._get_category_colors([item[0] for item in filtered_categories])
        }
        
        # Prepare location pie chart data (sorted, with Other)
        location_pie_data = {
            "labels": [item[0] for item in filtered_locations],
            "values": [item[1] for item in filtered_locations],
            "colors": self._get_location_colors([item[0] for item in filtered_locations])
        }
    ...
    def generate_hr_report_pdf(self, report_data):
        ...
        if top_jobs:
            ...
            for i, job in enumerate(top_jobs, 1):
                ...
                <b>Quality Score:</b> {job['quality_score']}/100
                """
